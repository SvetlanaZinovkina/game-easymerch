function Game() {
				...
				for (var y = 0; y <= this.state.height; y++) {
						for (var x = 0; x <=this.state.width; x++) {
								...
								switch (this.state.map[y][x]) {
										case 'W': tileClass = 'tileW'; break;
										case '.': tileClass = 'tile'; break;
										case 'P': tileClass = 'tileP'; break;
										case 'HP': tileClass = 'tileHP'; break;
										case 'SW': tileClass = 'tileSW'; break;
										case 'E': tileClass = 'tileE'; break;
										default: tileClass = 'tile'; break;
								}
								var tile = $('<div class="tile ' + tileClass + '" style="left:' + (x * 25) + 'px;top:' + (y * 25) + 'px;"></div>');

								if (this.state.map[y][x] === 'P') {
										var healthBarWidth = Math.max(0, this.state.heroHealth) + '%';
										tile.append('<div class="health" style="width:' + healthBarWidth + ';"></div>');
								}

								if (this.state.map[y][x] === 'E') {
										var enemy = this.state.enemies.find(e => e.x === x && e.y === y);
										var healthEnemiesWidth = Math.max(0, enemy.health) + '%';
										tile.append('<div class="health" style="width:' + healthEnemiesWidth + '; background-color: red;"></div>');
								}
		...
		this.generateRooms = function () {
				...
				for (var i = 0; i < roomCount; i++) {
						...
						this.state.rooms.push({ x: startX + Math.floor(roomWidth / 2), y: startY + Math.floor(roomHeight / 2) });
				}
		};
		...
		this.placeHeroAndEnemies = function () {
				...
				while (enemies > 0) {
						...
						if (this.state.map[y][x] === '.') {
								...
								this.state.enemies.push({ x: x, y: y, axis: axis, direction: direction, health: enemyHealth });
								enemies--;
						}
				}
		};
		...
		this.moveHero = function (dx, dy) {
				...
				if (newX >= 0 && newX < this.state.width && newY >= 0 && newY < this.state.height && this.state.map[newY][newX] !== 'W') {
		...
		this.attack = function() {
				...
				for (var i = 0; i < neighbors.length; i++) {
						...
						if (neighbor.x >= 0 && neighbor.x < this.state.width && neighbor.y >= 0 && neighbor.y < this.state.height) {
								var enemy = this.state.enemies.find(e => e.x === neighbor.x && e.y === neighbor.y);
								if (enemy) {
		...
		this.moveEnemies = function() {
				for (var i = 0; i < this.state.enemies.length; i++) {
						...
						if (newX >= 0 && newX < this.state.width && newY >= 0 && newY < this.state.height && this.state.map[newY][newX] === '.') {
		...
		this.updateInventory = function () {
				...
				var potionsDiv = $('<div class="inventory-item" >Зелья: <span id="potions-count">' + this.state.potions + '</span></div>');
				inventory.append(potionsDiv);

				var swordsDiv = $('<div class="inventory-item">Мечи: <span id="swords-count">' + this.state.swords + '</span></div>');
				inventory.append(swordsDiv);

				var enemiesDiv = $('<div class="inventory-item">Враги: <span id="enemies-count">' + this.state.enemies.length + '</span></div>');
				inventory.append(enemiesDiv);

				var powerUpStatus = this.state.heroPowerUp ? 'Да' : 'Нет';
				var powerDiv = $('<div class="inventory-item">Усиление: <span id="powerboost-count">' + powerUpStatus + '</span></div>');
		...
		$(document).keydown(function(e) {
				switch (e.which) {
						case 87: self.moveHero(0, -1); break;
						case 65: self.moveHero(-1, 0); break;
						case 83: self.moveHero(0, 1); break;
						case 68: self.moveHero(1, 0); break;
						case 32: self.attack(); break;
				}
		});
}
